...
struct ChainDetails {
  ...
  bool isNative; // Whether or not the token is the native gas token of the chain. E.g. Are you deploying an xERC20 for MATIC in Polygon?
...
contract XERC20Deploy is Script, ScriptingLibrary {
  ...
  function run() public {
    ...
    for (uint256 i; i < _chainAmount; i++) {
      ...
      address _lockbox;
      if (_chainDetails.erc20 != address(0) && !_chainDetails.isNative) {
        _lockbox = factory.deployLockbox(_xerc20, _chainDetails.erc20, _chainDetails.isNative);
      }
